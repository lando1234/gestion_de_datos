...
CREATE TABLE [NO_SRTA_E_GATOREI].[PROVEEDORES](
	...
	[NOMBRE_CONTACTO] [NVARCHAR](255) DEFAULT '',
	[RUBRO_ID] INT FOREIGN KEY REFERENCES [NO_SRTA_E_GATOREI].[RUBROS] (RUBRO_ID) ,
	[MAIL] [NVARCHAR](255) NOT NULL DEFAULT '',
	[TELEFONO] [NUMERIC](18, 0) NOT NULL,
	[BAJA_LOGICA] [BIT] NOT NULL DEFAULT 1,
	[USUARIO_ID] INT FOREIGN KEY REFERENCES [NO_SRTA_E_GATOREI].[USUARIOS](USUARIO_ID),
	[DIRECCION_ID] INT NOT NULL FOREIGN KEY REFERENCES [NO_SRTA_E_GATOREI].[DIRECCIONES](DIRECCION_ID)
...
INSERT INTO [NO_SRTA_E_GATOREI].[CLIENTES]
           ...
           ,[USUARIO_ID]
           ,[DIRECCION_ID])
     SELECT M.CLI_DNI, M.CLI_NOMBRE, M.CLI_APELLIDO, ISNULL(M.CLI_MAIL,''), M.CLI_TELEFONO, M.CLI_FECHA_NAC, NULL , D.DIRECCION_ID
...
INSERT INTO [NO_SRTA_E_GATOREI].[PROVEEDORES] 
           ...
           ,[USUARIO_ID]
           ,[DIRECCION_ID])
    SELECT M.PROVEE_CUIT, M.PROVEE_RS, '', R.RUBRO_ID,'', M.PROVEE_TELEFONO, NULL , D.DIRECCION_ID
...
GO
INSERT INTO [NO_SRTA_E_GATOREI].[ROLES](NOMBRE) VALUES ('ADMINISTRADOR_GENERAL')
GO
...
GO
INSERT INTO [NO_SRTA_E_GATOREI].PERMISOS_ROLES(ROL_ID,PERMISO_ID)
SELECT 1 ,PERMISO_ID FROM [NO_SRTA_E_GATOREI].PERMISOS
GO
...
BEGIN
	...
	VALUES
		(@DNI, @NOMBRE, @APELLIDO, @MAIL, @TELEFONO, convert(datetime,@FECHA_NACIMIENTO,121), @USER_ID, @ID_NUEVO)
	...
	VALUES(convert(datetime,@FECHA_ACTUAL,121), (SELECT TIPO_PAGO_ID
...
BEGIN
	...
	VALUES
		(@DNI, @NOMBRE, @APELLIDO, @MAIL, @TELEFONO,convert(datetime,@FECHA_NACIMIENTO,121), @ID_NUEVO)
...
BEGIN
	...
	VALUES
		(convert(datetime,@FECHA,121), @TIPO_PAGO_ID, @MONTO, @CLIENTE_ID, @TARJETA_ID)
	IF @@ERROR <> 0
		BEGIN
		ROLLBACK TRANSACTION
		RETURN @@ERROR
...
BEGIN
	...
	FROM [NO_SRTA_E_GATOREI].FACTURAS;
	...
	FROM [NO_SRTA_E_GATOREI].COMPRAS C, [NO_SRTA_E_GATOREI].OFERTAS O
	WHERE C.OFERTA_ID = O.OFERTA_ID
		AND NOT EXISTS ( SELECT 1
		FROM [NO_SRTA_E_GATOREI].FACTURAS_COMPRAS FC
		WHERE FC.COMPRA_ID = C.COMPRA_ID )
...
AS
BEGIN
	DECLARE @CREDITO DECIMAL(18,2),@PRECIO DECIMAL(18,2),@STOCK NUMERIC(18,0), @MAX INT,@CANT INT, @COMPRADOS INT;

	SELECT @CREDITO = CREDITO
	FROM [NO_SRTA_E_GATOREI].CLIENTES
	WHERE CLIENTE_ID = @CLIENTE_ID;

	SELECT @PRECIO = PRECIO_OFERTA, @MAX = MAXIMO_USUARIO, @STOCK = CANTIDAD
	FROM [NO_SRTA_E_GATOREI].OFERTAS
	WHERE OFERTA_ID = @OFERTA_ID;

	SELECT @COMPRADOS = COUNT(*)
...
BEGIN
		...
		VALUES
			(@CLIENTE_ID, @OFERTA_ID, convert(datetime,@FECHA_COMPRA,121));
...
ELSE 
BEGIN
		UPDATE [NO_SRTA_E_GATOREI].CUPONES SET FECHA_CONSUMO = convert(datetime,@FECHA_CONSUMO,121), ESTADO='ENTREGADO' WHERE CUPON_ID = @CODIGO_CUPON
		RETURN 0
	END
END